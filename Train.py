...
# @lp
def trainModel(feature_maps,margin,output_dim,same_factor):
    ################ **Loading DataSet** ##################
    start = time()
    directory ='subset'
    ...
    net = Net(output_dim,feature_maps)
    net.to(device)
    net.train()
    ...
    criterion = nn.HingeEmbeddingLoss(margin = margin,reduction='none')
    ...
    w.add_experiment_parameter("Feature Maps",feature_maps)
    w.add_experiment_parameter("Margin",margin)
    w.add_experiment_parameter("Output Dim",output_dim)
    w.add_experiment_parameter("Same Factor",same_factor)
    ...
    while epoch <epochs:
        ...
        label_count = 0
        one_img_labels = []
        while img_count < same_factor*batch_size:
